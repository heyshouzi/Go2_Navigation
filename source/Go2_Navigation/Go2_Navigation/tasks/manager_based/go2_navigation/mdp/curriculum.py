...
def adaptive_collision_penalty_curriculum(
    ...
    if current_rl_iteration - state["initialized_at_iteration"] < warmup_iterations:
        ...
        if state["current_weight"] != weight_levels[0]:
            ...
            if hasattr(env, "reward_manager"):
                try:
                    ...
                    print(
                        f"🔧 [Collision Curriculum] 预热期强制保持权重: {state['current_weight']}"
                    )
        ...
        return {
            ...
            "warmup": float(
                warmup_iterations
                - (current_rl_iteration - state["initialized_at_iteration"])
            ),
        }

    # 每N个RL iterations评估一次，且保证样本量足够
    if (
        iterations_since_last_eval >= eval_interval
        and state["episode_count"] >= min_episodes_per_eval
    ):
        # 计算成功率
        if state["episode_count"] > 0:
            success_rate = state["success_count"] / state["episode_count"]
        ...
        if (
            ...
            and success_rate >= success_thresholds[current_level]
        ):
            # 升级到下一档
            ...
            if hasattr(env, "reward_manager"):
                try:
                    ...
                    print(
                        f"   成功率: {success_rate:.2%} (阈值: {success_thresholds[current_level]:.2%})"
                    )
