...
parser = argparse.ArgumentParser(
    description="Play navigation with MLP encoder (RSL-RL)"
)
parser.add_argument(
...
parser.add_argument(
    "--enable_path_visualization",
    action="store_true",
    default=False,
    help="Enable path visualization (shows robot trajectory as green lines). May impact performance.",
)
...
def main(
    ...
    if args_cli.use_pretrained_checkpoint:
        ...
        model_files = glob.glob(
            os.path.join(log_root_path, "**/model_*.pt"), recursive=True
        )
        if model_files:
    ...
    # 🆕 路径可视化：记录和可视化go2走过的路线（可选）
    enable_path_viz = args_cli.enable_path_visualization
    ...
    if enable_path_viz:
        ...
        # 🔧 记录每个环境已可视化的路径点数量（用于增量更新）
        visualized_point_count = defaultdict(int)  # {env_idx: count}
        # 🔧 限制路径点最大数量，避免无限增长
        MAX_PATH_POINTS = 500  # 最多保留500个点
        ...
        def update_path_visualization_incremental(env_idx: int, points: list):
            """🔧 优化：增量更新路径可视化 - 只添加新的线段，不重建全部"""
            if len(points) < 2:
                return
            ...
            if len(points) > MAX_PATH_POINTS:
                # 移除最旧的点
                points_to_remove = len(points) - MAX_PATH_POINTS
                points[:] = points[points_to_remove:]
                # 如果路径被截断，需要重建可视化
                parent_path = f"/World/PathVisualization/env_{env_idx}"
                if is_prim_path_valid(parent_path):
                    delete_prim(parent_path)
                visualized_point_count[env_idx] = 0

            # 使用线条段（Line segments）绘制路径
            parent_path = f"/World/PathVisualization/env_{env_idx}"
            ...
            # 从上次可视化的点开始，添加新的线段
            start_idx = max(
                0, visualized_point_count[env_idx] - 1
            )  # -1 因为需要连接上一个点
            ...
            for i in range(start_idx, len(points) - 1):
                ...
                # 如果线段已存在，更新它；否则创建新的
                if is_prim_path_valid(line_path):
                    line_prim = UsdGeom.BasisCurves.Get(stage, line_path)
                else:
                ...
                # 设置颜色为绿色
                if not line_prim.GetDisplayColorAttr():
                    color_attr = line_prim.CreateDisplayColorAttr()
                else:
                    color_attr = line_prim.GetDisplayColorAttr()
                color_attr.Set([Gf.Vec3f(0.0, 1.0, 0.0)])

                # 设置宽度
                if not line_prim.GetWidthsAttr():
                    widths_attr = line_prim.CreateWidthsAttr()
                else:
                    widths_attr = line_prim.GetWidthsAttr()
                widths_attr.Set([0.05])

            # 更新已可视化的点数量
            visualized_point_count[env_idx] = len(points)
        ...
        timestep = 0
        last_update_step = 0
        update_interval = 20  # 🔧 优化：增加更新间隔到20步，减少更新频率
        last_reset_buf = None  # 用于检测环境重置
        path_record_interval = 5  # 🔧 优化：增加记录间隔到5步，减少路径点数量
        last_path_record_step = 0

        print("[INFO] 🎨 已启用路径可视化功能，将显示go2走过的路线（绿色线条）")
        print(
            f"[INFO] 路径可视化优化：最大点数={MAX_PATH_POINTS}, 更新间隔={update_interval}步, 记录间隔={path_record_interval}步"
        )
    else:
        # 不启用路径可视化时，只初始化基本变量
        robot = env.unwrapped.scene["robot"]
        obs = env.get_observations()
        timestep = 0
        print("[INFO] 路径可视化已禁用（使用 --enable_path_visualization 启用）")
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            if enable_path_viz:
                        ...
                        if reset_mask.any():
                            for env_idx in range(reset_mask.shape[0]):
                                if reset_mask[env_idx]:
                                    ...
                                    # 清除可视化
                                    prim_path = (
                                        f"/World/PathVisualization/env_{env_idx}"
                                    )
                ...
                if robot.is_initialized and (
                    timestep - last_path_record_step >= path_record_interval
                ):
            ...
            if enable_path_viz:
                ...
                if timestep - last_update_step >= update_interval:
                    for env_idx, points in path_points.items():
                        if len(points) > 1:
                            update_path_visualization_incremental(env_idx, points)
