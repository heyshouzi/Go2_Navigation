...
class PreTrainedPolicyAction(ActionTerm):
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        # vx: [-1.0, 1.0] m/s (前后)
        # vy: [-0.8, 0.8] m/s (左右)
        # vyaw: [-1.0, 1.0] rad/s (旋转)
        self._raw_actions[:, 0] = torch.clamp(actions[:, 0], -1.0, 1.0)   # vx
        self._raw_actions[:, 1] = torch.clamp(actions[:, 1], -0.8, 0.8)   # vy
        self._raw_actions[:, 2] = torch.clamp(actions[:, 2], -1.0, 1.0)   # vyaw
