...
def obstacle_mlp_encoding(
    ...
    is_missed = torch.isinf(ray_hits_w).any(dim=-1) | torch.isnan(ray_hits_w).any(dim=-1)  # (env, rays)
    
    # 对于未命中的射线，将相对向量设置为零，避免在坐标变换时产生 NaN
    # 后续会用 max_distance 替换这些值
    relative_vec_w = torch.where(
        is_missed.unsqueeze(-1),
        torch.zeros_like(relative_vec_w),
        relative_vec_w
    )
    ...
    quat_w_expanded = quat_w.unsqueeze(1).expand(-1, num_rays, -1).reshape(-1, 4)  # (env * rays, 4)
    
    # 应用逆旋转：从世界坐标系转换到传感器坐标系
    relative_vec_b_flat = math_utils.quat_apply_inverse(quat_w_expanded, relative_vec_w_flat)
    relative_vec_b = relative_vec_b_flat.reshape(num_envs, num_rays, 3)  # (env, rays, 3)
