...
class PolicyWithLidarEncoder(nn.Module):
    ...
    def __init__(self, policy, normalizer=None):
        ...
        if hasattr(policy, 'lidar_encoder'):
            ...
            print(f"✅ Copied LiDAR encoder: {policy.num_lidar_obs} → {self.lidar_encoder.output_dim}")
        else:
        ...
        print(f"   Actor input: {self.num_basic_obs + self.lidar_encoder.output_dim} dims")
...
def export_mlp_policy_as_jit(
    ...
    test_obs = torch.randn(1, deployment_policy.num_basic_obs + deployment_policy.num_lidar_obs)
...
def export_policy_from_checkpoint(
    ...
    print("    runner.save(os.path.join(runner.log_dir, f'model_{runner.current_learning_iteration}.pt'))")
