...
def obstacle_mlp_encoding(
    ...
    encoder_output_dim: int = 360,
) -> torch.Tensor:
    """
    返回每条激光的距离值，在机器人本体坐标系下的平面距离。
    支持LidarSensorCfg和RayCaster两种传感器类型。
    未命中的射线 distance = sensor.cfg.max_distance

    Args:
        env: 环境实例
        sensor_cfg: 传感器配置
        encoder_output_dim: 编码器输出维度（用于配置记录，实际输出维度由传感器决定）
    ...
    # 检查是否为LidarSensor（有get_distances方法）
    if hasattr(sensor, 'get_distances'):
        # 使用LidarSensorCfg的get_distances方法
        # 获取所有环境的数据
        env_ids = torch.arange(env.num_envs, device=env.device)
        distances = sensor.get_distances(env_ids)  # (num_envs, num_rays)
        
        # 处理无效值（inf, nan等）
        max_d = sensor.cfg.max_distance
        distances = torch.where(
            torch.isinf(distances) | torch.isnan(distances) | (distances > max_d),
            torch.full_like(distances, max_d),
            distances,
        )
        
        # 确保距离在有效范围内 [0, max_d]
        distances = torch.clamp(distances, 0.0, max_d)
        ...
        if not hasattr(env, "_lidar_config"):
            ...
            print(f"[LiDAR] {distances.shape[1]} rays → distance only, maxD={max_d}")
